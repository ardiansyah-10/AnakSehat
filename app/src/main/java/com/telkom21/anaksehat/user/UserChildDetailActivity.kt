...
import com.telkom21.anaksehat.databinding.ActivityUserChildDetailBinding
...
class UserChildDetailActivity : AppCompatActivity() {
    private lateinit var binding: ActivityUserChildDetailBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityUserChildDetailBinding.inflate(layoutInflater)
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        for (childSnapshot in snapshot.children) {
                            val childData = childSnapshot.getValue(UserChildDataClass::class.java)
                            if (childData != null) {
                                binding.nama.text = childData.nama
                                binding.umur.text = childData.umur
                                binding.beratBadan.text = childData.berat
                                binding.tinggiBadan.text = childData.tinggi
                                binding.jenisKelamin.text = childData.jenisKelamin
                        ...
                        Toast.makeText(this@UserChildDetailActivity, "Child data not found", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onCancelled(error: DatabaseError) {
                    Toast.makeText(this@UserChildDetailActivity, "Failed to load data", Toast.LENGTH_SHORT).show()
                }
            })
        }

        badutaRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    fetchData(badutaRef)
                } else {
    ...
    private fun fetchHistory(childName: String, parentId: String) {
        val database = FirebaseDatabase.getInstance().reference
        val badutaHistoryRef = database.child("users").child(parentId).child("baduta").orderByChild("nama").equalTo(childName)
        val balitaHistoryRef = database.child("users").child(parentId).child("balita").orderByChild("nama").equalTo(childName)
            ...
            ref.addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        historyList.clear()
                        for (childSnapshot in snapshot.children) {
                            val historyRef = childSnapshot.child("history")
                            for (dateSnapshot in historyRef.children) {
                                val berat = dateSnapshot.child("berat").getValue(String::class.java) ?: "Unknown"
                                val tinggi = dateSnapshot.child("tinggi").getValue(String::class.java) ?: "Unknown"
                                val tanggal = dateSnapshot.child("tanggal").getValue(String::class.java) ?: "Unknown"
                                val historyItem = HistoryDataClass(tanggal, berat, tinggi)
                        ...
                        historyAdapter.mListener = object : HistoryAdapter.OnItemClickListener {
                            override fun onItemClick(position: Int) {
                                ...
                                val intent = Intent(this@UserChildDetailActivity, PastHistoryActivity::class.java).apply {
                        ...
                        Toast.makeText(this@UserChildDetailActivity, "No history data available", Toast.LENGTH_SHORT).show()
                    } 
                }

                override fun onCancelled(error: DatabaseError) {
                    Toast.makeText(this@UserChildDetailActivity, "Failed to load history", Toast.LENGTH_SHORT).show()
                }
            })
        }

        badutaHistoryRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
    ...
    private fun calculateAndSetBMI(childData: UserChildDataClass) {
        ...
        if (berat != null && tinggi != null && umur != null && gender != null) {
    ...
    private fun giveSuggestion(category: String){
        ...
        when (category) {
            "Gizi Buruk" -> saran.text = getString(R.string.saranGiziBuruk)
            "Gizi Kurang" -> saran.text = getString(R.string.saranGiziKurang)
            "Gizi Baik" -> saran.text = getString(R.string.saranGiziBaik)
            "Beresiko gizi lebih" -> saran.text = getString(R.string.saranResikoGiziLebih)
            "Gizi Lebih" -> saran.text = getString(R.string.saranGiziLebih)
            "Obesitas" -> saran.text = getString(R.string.saranObesitas)
            else -> saran.text = "-"
        }
    }
    ...
    private fun setupLineChart(lineChart: LineChart) {
        ...
        lineChart.axisLeft.apply {
            axisLineColor = ContextCompat.getColor(this@UserChildDetailActivity, R.color.white)
            axisLineWidth = 3f
            textColor = ContextCompat.getColor(this@UserChildDetailActivity, R.color.white)
            gridColor = ContextCompat.getColor(this@UserChildDetailActivity, R.color.sec_white)
        }
    ...
    private fun updateLineChart(historyList: List<HistoryDataClass>) {
        val entries = historyList.mapIndexed { index, data ->
            data.berat?.let { Entry(index.toFloat(), it.toFloat()) }
        }

        val lineDataSet = LineDataSet(entries, "Berat over Time")
